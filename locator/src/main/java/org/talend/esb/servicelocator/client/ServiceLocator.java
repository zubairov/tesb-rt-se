 ...
 * first {@link #connect() connect} to the Service Locator to get a session assigned at the server. Once the
 * connection is
 * established the client will periodically send heart beats to the server to keep the session alive.
 * The session gets invalidated if the client is
 * destroyed, disconnected, or fails to successfully send the heart beat for a period of time defined by
 * the {@link #setSessionTimeout(int) session timeout parameter}.
 * <p>
 * The Service Locator provides the following operations.
 * <ul>
 * <li>Register an endpoint for a specific service. If registered as non persistent an endpoint is marked
 * live as long as the session of this client is alive or it is explicitly unregistered. If registered
 * as persistent the endpoint is marked live until the endpoint is explicitly unregistered.</li>
 *
 * <li>Unregister an endpoint for a specific service. The endpoint is marked as non live, but still
 * in the list of all endpoints for a service.</li>
 *
 * <li>Remove an endpoint for a specific service. The endpoint is removed from 
 * the list of endpoints for the given service.</li>
 *
 * <li>Look up all live endpoints for a specific service that were registered before by other clients.</li>
 * 
 ...
 * </ul>
 * 
 * To ensure that all available endpoints are re-registered when the client reconnects after a
 * session expired a {@link PostConnectAction} should be {@link #setPostConnectAction(PostConnectAction)
 * set} that registers all endpoints again and so marks them as live.
 */
public interface ServiceLocator {
    ...
    /**
     * For a given service register the endpoint of a concrete provider of this service. The endpoint is
     * marked as live as long as this client is connected and the session on the server is valid.
     * <p>
    ...
    /**
     * For a given service register the endpoint of a concrete provider of this service. The endpoint is
     * marked as live as long as this client is connected and the session on the server is valid. In
     * addition a set of custom properties is defined for the endpoint. They may be used as additional
     * selection criteria during the lookup.
     * 
     ...
     *             the endpoint to register, must not be <code>null</code>
     * @param properties
     *             custom properties of the endpoint that can be used as additional selection criteria
     *             when doing a lookup.
     * @throws ServiceLocatorException
     *             the server returned an error
     * @throws InterruptedException
    ...
    /**
     * For a given service register the endpoint as defined in the given {@link EndpointProvider
    ...
    /**
     * Remove the given endpoint from the list of endpoints of the given service. When calling
     ...
     * 
     * @param serviceName
     *            the name of the service the endpoint is removed from, must not be <code>null</code>
     * @param endpoint
     ...
     * 
     * @param serviceName
     *             the name of the service for which the endpoint is registered, must not be <code>null</code>
     * @param endpoint
     *             the endpoint for which to return the information, must not be <code>null</code>
     * @return the end point information, may be <code>null</code> if the endpoint was not registered before
     * @throws ServiceLocatorException
     *             the server returned an error
     * @throws InterruptedException
     *             the current <code>Thread</code> was interrupted when waiting for a response of the
     ...
     * 
     * @param serviceName
     *             the name of the service for which the endpoints ares registered, must not be
     *             <code>null</code>
     * @return the list of end point informations, may be empty if no the endpoints are registered for
     *         the service.
     * @throws ServiceLocatorException
     *             the server returned an error
     * @throws InterruptedException
    ...
    /**
     * For the given service return all endpoints that are currently registered at the Service Locator
     * and where the custom properties match the given matcher. 
     * 
     * @param serviceName
     *            the name of the service for which to get the endpoints, must not be <code>null</code>
     * @param matcher
     *            custom properties of the endpoints returned match the criterias specified by this matcher
     * @return a possibly empty list of endpoints
     * @throws ServiceLocatorException
     *             the server returned an error
     * @throws InterruptedException
     ...
     * 
     * @param postConnectAction the action to be executed, must not be <code>null</code>.
     */
