...
public class EndpointResolver {
	...
	private static final Logger LOG = Logger.getLogger(EndpointResolver.class.getName());
	...
	public EndpointResolver(QName serviceName, String locatorEndpoints) {
		LOG.log(Level.INFO, "Creating EndpointResolver object for " + serviceName.toString() + " service.");
	...
	private List<String> receiveEndpointsList() {
		...
		LOG.log(Level.INFO, "Received list of endpoints: " + endpointsList.toString());
	...
	public String selectEndpoint() {
		if (endpointsList.isEmpty()) {
			...
			LOG.log(Level.INFO,	"Selected endpoint: " + endpointsList.get(endpointIndex));
	...
	public void refreshEndpointsList() {
		try {
			...
			if (LOG.isLoggable(Level.SEVERE)) {
				LOG.log(Level.SEVERE, "Can not refresh list of endpoints due to unknown exception");
			}
		}
	}
}
