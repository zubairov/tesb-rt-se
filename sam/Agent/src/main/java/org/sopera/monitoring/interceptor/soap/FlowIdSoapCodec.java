package org.sopera.monitoring.interceptor.soap;

import static org.sopera.monitoring.helper.FlowIdHelper.FLOW_ID_QNAME;
...
public class FlowIdSoapCodec extends AbstractSoapInterceptor implements MessageContextCodec  {
	...
	public void handleMessage(SoapMessage soapMessage) throws Fault {
			logger.finest("FlowIdSoapCodec Interceptor called. isOutbound: " + MessageUtils.isOutbound(soapMessage) + ", isRequestor: " + MessageUtils.isRequestor(soapMessage));
	...
	public void writeFlowId(Message message, String flowId) {
		...
		Header flowIdHeader;
		try {
			flowIdHeader = new Header(FLOW_ID_QNAME, flowId,
					new JAXBDataBinding(String.class));
			headers.add(flowIdHeader);
			logger.fine("Stored flowId '" + flowId + "' in soap header: "
						+ FLOW_ID_QNAME.toString());
